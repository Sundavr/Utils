...
/**
 * Timer improved to support defined number of repetitions for TimerTasks.
 * 
 ...
 *
 * @author  Josh Bloch + Gousset Johan
 * @see     TimerTask
...
public class MyTimer {
    ...
    public void schedule(MyTimerTask task, long delay) {
        ...
        if (task.repeatsLeft > 1)
            sched(task, System.currentTimeMillis()+delay, delay);
        else
    ...
    public void schedule(MyTimerTask task, Date time) {
        if (task.repeatsLeft > 1)
            sched(task, time.getTime(), time.getTime());
        else
        ...
        synchronized(this.queue) {
            ...
            synchronized(task.lock) {
                ...
                if (task.repeatsLeft == 0)
                    throw new IllegalStateException(
                        "All the task repeats have been done");
    ...
    public void cancel() {
        synchronized(this.queue) {
            ...
            this.queue.clear();
            this.queue.notify();  // In case queue was already empty.
        }
    }
    ...
    public int purge() {
        ...
        synchronized(this.queue) {
            for (int i = this.queue.size(); i > 0; i--) {
                if (this.queue.get(i).state == MyTimerTask.CANCELLED) {
    ...
    /**
     * Return the Timer's thread name.
     * @return the Timer's thread name
     */
    ...
    /**
     * Change the name of this Timer's thread;
    ...
    /**
     * Return true if this Timer's thread is a daemon, otherwise false.
     * @return true if this Timer's thread is a daemon, otherwise false
     */
    ...
    /**
     * Change the daemon status of this Timer's thread.
     * @param isDaemon true if the associated thread should run as a daemon.
     */
    public void setThreadDaemon(boolean isDaemon) {
        this.thread.setDaemon(isDaemon);
    }
    ...
    /**
     * Return a list containing all the current scheduled tasks.
     * @return a list containing all the current scheduled tasks
     */
    public ArrayList<MyTimerTask> getScheduledTasks() {
        ArrayList<MyTimerTask> timerTasksList = new ArrayList<>();
        for (int i=0; i<this.queue.size(); i++) {
            timerTasksList.add(this.queue.get(i));
        }
        ...
        while (true) {
            try {
                ...
                synchronized(this.queue) {
                    ...
                    while (this.queue.isEmpty() && newTasksMayBeScheduled)
                        this.queue.wait();
                    ...
                    synchronized(task.lock) {
                        ...
                        if (taskFired = (executionTime<=currentTime)) {
                            if (task.repeatsLeft > 0) task.repeatsLeft--;
                            if ((task.repeatsLeft == 0) || (task.period == 0)) { // Non-repeating, remove
                                this.queue.removeMin();
                                task.state = MyTimerTask.EXECUTED;
    ...
    private void fixUp(int k) {
        while (k > 1) {
            ...
            if (this.queue[j].nextExecutionTime <= this.queue[k].nextExecutionTime)
                break;
            MyTimerTask tmp = this.queue[j];  this.queue[j] = this.queue[k]; this.queue[k] = tmp;
            k = j;
        }
    }
